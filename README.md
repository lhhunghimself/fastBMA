...
fastBMA has been incorporated into the networkBMA package. However, the package is in beta and can only be installed on Linux systems. This is due to the requirement for OpenBLAS which is difficult to provide in R without recompiling R. There is no MPI support for the R version but multithreading is available through openMP.
##Sample usage#
